...
let pick_at l i =
  let rec pick_at_aux l i acc =
    match l with
      | h::t when i = 0 -> (acc @ t, h)
      | h::t -> pick_at_aux t (i - 1) (h::acc)
...
let newDeck =
  let rec newDeck_aux cards acc =
    ...
    | cards ->
      ...
      let (remaining_cards, random_card) = pick_at cards index in
      newDeck_aux remaining_cards (random_card::acc) in
  newDeck_aux Card.all []
...
let drawCard = function
  | h::t -> (h, t)
  | [] -> raise (Failure "No card to draw here...")